...
void TeapotRenderer::Init()
{
    ...
    //Load shader
    LoadShaders( &shader_param_, vsh_source,
            fsh_source );
                                 ...
                                 std::string strFsh )
{
    ...
    if( !ndk_helper::shader::CompileShader( &vert_shader, GL_VERTEX_SHADER, strVsh.data(), (GLint)strVsh.length() ) )
    {
    ...
    if( !ndk_helper::shader::CompileShader( &frag_shader, GL_FRAGMENT_SHADER, strFsh.data(), (GLint)strFsh.length() ) )
    {
        glDeleteProgram( program );
        return false;
    }
